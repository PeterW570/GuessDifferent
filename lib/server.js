...
function handleUniqueQuestion() {
	...
	for (let client of clients) {
		...
		if (data.pendingAnswer) {
			const ans = data.pendingAnswer.trim().toLowerCase();
			if (answerFrequencies[ans])
				answerFrequencies[ans]++;
			else answerFrequencies[ans] = 1;
		}
	}
	for (let client of clients) {
		const data = clientData.get(client);
		...
		const gotCorrectAnswer =
			data.pendingAnswer &&
			gotUniqueAnswer &&
			currentQuestion.answer.includes(data.pendingAnswer.trim().toLowerCase());
